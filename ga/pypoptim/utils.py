...
def init_population_from_backup(backup, config):
    ...
    assert (backup[0]['state'].shape ==
            config['runtime']['states_initial'].shape)
...
def run_model_ctypes(C, config):
    ...
    A = config['kwargs']['algebraic']['value']
    #print(A)
...
def update_phenotype(organism, config):
    ...
    for exp_cond_name in config['experimental_conditions']:
        ...
        organism['phenotype'][exp_cond_name] = pd.DataFrame(
            res.T, columns=config['columns_model'])
