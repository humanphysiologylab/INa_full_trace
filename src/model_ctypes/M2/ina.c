...
void compute_algebraic(const realtype time,  N_Vector STATES, N_Vector CONSTANTS,  N_Vector ALGEBRAIC){
  ...
  Ith(ALGEBRAIC,0) = 1/(Ith(CONSTANTS,2) * exp(Ith(STATES,2)/Ith(CONSTANTS,5)) + Ith(CONSTANTS,3)*exp(- Ith(STATES,2)/Ith(CONSTANTS,4)));
  
  // tau_h = 1 / (a0_h * exp(-v_m / s_h) + b0_h * exp(v_m / delta_h))
  Ith(ALGEBRAIC,1) = 1/(Ith(CONSTANTS,6) * exp(- Ith(STATES,2)/Ith(CONSTANTS,9)) + Ith(CONSTANTS,7)*exp(Ith(STATES,2)/Ith(CONSTANTS,8)));
