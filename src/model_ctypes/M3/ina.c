...
void compute_algebraic(const realtype time,  N_Vector STATES, N_Vector CONSTANTS,  N_Vector ALGEBRAIC){
  ...
  Ith(ALGEBRAIC,0) =  1 / ((1 + exp(-(Ith(CONSTANTS,14) + Ith(STATES,2))/Ith(CONSTANTS,16)))*Ith(CONSTANTS,2)*exp( Ith(STATES,2) /Ith(CONSTANTS,3) ));
  
  // tau_h = 1 / ((1 + exp((v_half_h + v_m) / k_h)) * a0_h * exp( -v_m / s_h))
  Ith(ALGEBRAIC,1) = 1 / ((1 + exp((Ith(CONSTANTS,15) + Ith(STATES,2))/Ith(CONSTANTS,17)))*Ith(CONSTANTS,4)*exp(-Ith(STATES,2)/Ith(CONSTANTS,5)));
