...
void compute_algebraic(const realtype time,  N_Vector STATES, N_Vector CONSTANTS,  N_Vector ALGEBRAIC){
  ...
  realtype s_m = Ith(CONSTANTS,3);
  realtype tau_m_const = Ith(CONSTANTS,4);    
  realtype a0_h = Ith(CONSTANTS,5);
  realtype s_h = Ith(CONSTANTS,6);
  realtype tau_h_const = Ith(CONSTANTS,7);
  realtype a0_j = Ith(CONSTANTS,8);
  realtype s_j = Ith(CONSTANTS,9);
  realtype tau_j_const = Ith(CONSTANTS,10);
  realtype R = Ith(CONSTANTS,11);
  realtype R_f = Ith(CONSTANTS,12);
  realtype g_max = Ith(CONSTANTS,13);
  realtype g_leak = Ith(CONSTANTS,14);
  realtype v_half_m = Ith(CONSTANTS,16);
  realtype v_half_h = Ith(CONSTANTS,17);
  realtype k_m = Ith(CONSTANTS,18);
  realtype k_h = Ith(CONSTANTS,19);
  realtype x_c_comp = Ith(CONSTANTS,20);
  realtype x_r_comp = Ith(CONSTANTS,21);
  realtype alpha = Ith(CONSTANTS,22);
  realtype v_off = Ith(CONSTANTS,23);
  realtype v_rev = Ith(CONSTANTS,24);
  realtype v_c = Ith(CONSTANTS,25);

  

  tau_m = tau_m_const + 1 / ((1 + exp(-(v_half_m + v_m)/k_m))*a0_m*exp( v_m /s_m ));
  tau_h = tau_h_const + 1 / ((1 + exp((v_half_h + v_m)/k_h))*a0_h*exp(-v_m/s_h));
  ...
  I_in = I_leak + I_Na + I_c + I_p - I_comp;//  I_leak + I_Na +I_c + I_p - I_comp
...
void compute_rates(const realtype time,  N_Vector STATES, N_Vector CONSTANTS,  N_Vector ALGEBRAIC, N_Vector RATES){
  ...
  realtype R = Ith(CONSTANTS,11);
  realtype R_f = Ith(CONSTANTS,12);
  realtype tau_z = Ith(CONSTANTS,15);
  realtype x_c_comp = Ith(CONSTANTS,20);
  realtype x_r_comp = Ith(CONSTANTS,21);
  realtype alpha = Ith(CONSTANTS,22);
  realtype v_off = Ith(CONSTANTS,23);
  ...
  Ith(RATES,0) = (v_c - v_comp)/(x_c_comp*c_m*x_r_comp*R*(1 - alpha));
  Ith(RATES,1) = (v_cp - v_p)/(c_p*R_f);
  Ith(RATES,2) = (v_p + v_off - v_m)/(R*c_m) - NM*(I_leak + I_Na)/c_m;
  Ith(RATES,3) = (m_inf - m)/tau_m;
  Ith(RATES,4) = (h_inf - h)/tau_h;
  Ith(RATES,5) = (h_inf - j)/tau_j;
  Ith(RATES,6) = (I_in - I_out)/tau_z;
  }