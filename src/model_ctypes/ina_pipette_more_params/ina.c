...
void compute_algebraic(const double time,  double *STATES, double *CONSTANTS,  double *ALGEBRAIC){//, double *RATES){
  ...
  ALGEBRAIC[4] = 1 / (1 + exp((CONSTANTS[23] + STATES[2])/CONSTANTS[25]));
  //h_inf = 1 / (1 + exp((v_half_h + v_m) / k_h));
  ALGEBRAIC[5] = CONSTANTS[31] + (CONSTANTS[31] - STATES[0])*(1/(1 - CONSTANTS[28]) - 1);
  //v_cp =  v_c + (v_c - v_comp)*(1/(1-alpha) - 1);
  ALGEBRAIC[6] = CONSTANTS[20] * STATES[2];
  //I_leak = g_leak * v_m;
  ALGEBRAIC[7] = CONSTANTS[19] * STATES[4] * pow(STATES[3],3) * STATES[5]* (STATES[2] - CONSTANTS[30]);
  //I_Na = g_max * h * pow(m,3) * (v_m - v_rev) * j ;

  ALGEBRAIC[8] = 1e9 * CONSTANTS[1] * ((STATES[1] + CONSTANTS[29] - STATES[2])/(CONSTANTS[17]*CONSTANTS[1]) - (1e-9)*(ALGEBRAIC[6] + ALGEBRAIC[7])/CONSTANTS[1]);// RATES[2];
  //I_c = 1e9 * constants['c_m'] * (np.diff(df.v_m) / dt)
  //ALGEBRAIC[8] = 1e9 * CONSTANTS[1] * ((ALGEBRAIC[5] + CONSTANTS[29] - STATES[2])/(CONSTANTS[17]*CONSTANTS[1]) - (1e-9)*(ALGEBRAIC[6] + ALGEBRAIC[7])/CONSTANTS[1]);// RATES[2];


  ALGEBRAIC[9] = 1e9 * CONSTANTS[0] *(ALGEBRAIC[5] - STATES[1])/(CONSTANTS[0]*CONSTANTS[18]);// RATES[1];
  //I_p = 1e9 * constants['c_p'] * (np.diff(df.v_p) / dt)
  ALGEBRAIC[10] = 1e9 * CONSTANTS[26] * CONSTANTS[1] *(CONSTANTS[31] - STATES[0])/(CONSTANTS[26]*CONSTANTS[1]*CONSTANTS[27]*CONSTANTS[17]*(1 - CONSTANTS[28]));// RATES[0]
...
void compute_rates(const double time,  double *STATES, double *CONSTANTS,  double *ALGEBRAIC, double *RATES){
  ...
  RATES[0] = (CONSTANTS[31] - STATES[0])/(CONSTANTS[26]*CONSTANTS[1]*CONSTANTS[27]*CONSTANTS[17]*(1 - CONSTANTS[28]));
  //v_comp[i] = v_c[i-1] + (v_comp[i-1] - v_c[i-1]) * exp(-dt / tau_srp);
  //tau_srp = x_r_comp*r_m; * x_c_comp*c_m; * (1 - alpha);
  RATES[1] = (ALGEBRAIC[5] - STATES[1])/(CONSTANTS[0]*CONSTANTS[18]);
  //v_p[i] = v_cp[i-1] + (v_p[i-1] - v_cp[i-1]) * exp(-dt / (r_p * c_p));

  RATES[2] = (STATES[1] + CONSTANTS[29] - STATES[2])/(CONSTANTS[17]*CONSTANTS[1]) - (1e-9)*(ALGEBRAIC[6] + ALGEBRAIC[7])/CONSTANTS[1];
  //v_m[i] = v_m[i-1] + (v_p[i-1] + v_off - v_m[i-1] ) * (dt / (r_m * c_m)) - 1e-9 * (I_Na[i-1] + I_leak[i-1]) * dt / c_m ;
  //it can be STATES[1] instead ALGEBRAIC[5]
  //RATES[2] = (ALGEBRAIC[5] + CONSTANTS[29] - STATES[2])/(CONSTANTS[17]*CONSTANTS[1]) - (1e-9)*(ALGEBRAIC[6] + ALGEBRAIC[7])/CONSTANTS[1];
  ...
  RATES[6] = (ALGEBRAIC[11] - STATES[6])/CONSTANTS[21];
