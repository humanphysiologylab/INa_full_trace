...
void compute_algebraic(const realtype time,  N_Vector STATES, N_Vector CONSTANTS,  N_Vector ALGEBRAIC){
  ...
  // I_in = 1e9 *(v_cp + v_off -v_m)/r_m - I_comp 
  Ith(ALGEBRAIC,11) = RNM*(Ith(ALGEBRAIC,5) + Ith(CONSTANTS,27) - Ith(STATES,2))/(Ith(CONSTANTS,15)) - Ith(ALGEBRAIC,10);
}
...
void compute_rates(const realtype time,  N_Vector STATES, N_Vector CONSTANTS,  N_Vector ALGEBRAIC, N_Vector RATES){
  ...
  // v_m = (v_cp + v_off - v_m ) / (r_m * c_m) - 1e-9 * (I_Na + I_leak) / c_m ;
  Ith(RATES,2) = (Ith(ALGEBRAIC,5) + Ith(CONSTANTS,27) - Ith(STATES,2))/(Ith(CONSTANTS,15)*Ith(CONSTANTS,1)) - NM*(Ith(ALGEBRAIC,6) + Ith(ALGEBRAIC,7))/Ith(CONSTANTS,1);
