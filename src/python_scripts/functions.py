...
def calculate_full_trace(y, *args):
    ...
    model = give_me_ina(filename_abs)
    ...
    if kwargs.get('log', None):
        ...
        # want to rescale [-25,25] -> [1,10] and in logarytmic it would be [0,1]
        x[27] = (x[27] - 5.5) * 50 / 9
    ...
    A_initial = np.zeros((initial_len, len(A)))
...
def OLD_calculate_full_trace(y, *args):
    ...
    status = ina.run(dt, S.values.copy(), x,
...
def give_me_ina(filename):
    ...
    # void compute_rates(const double time,  double *STATES, double *CONSTANTS,
    ...
    # void compute_algebraic(const double time,  double *STATES,
    ...
    #         int array_length, double *output_S, double *output_A)
    ina.run.argtypes = [
        np.ctypeslib.ndpointer(dtype=np.float64, ndim=1, flags='C_CONTIGUOUS'),
        np.ctypeslib.ndpointer(dtype=np.float64, ndim=1, flags='C_CONTIGUOUS'),
        np.ctypeslib.ndpointer(dtype=np.float64, ndim=1, flags='C_CONTIGUOUS'),
